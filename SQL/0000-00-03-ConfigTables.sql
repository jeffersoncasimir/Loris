...
INSERT INTO ConfigSettings (Name, Description, Visible, AllowMultiple, DataType, Parent, Label, OrderNumber) SELECT 'instrumentDataPath', 'Path to directory for uploaded instrument data csv files', 1, 0, 'text', ID, 'Instrument Data Upload Path', 15 FROM ConfigSettings WHERE Name="paths";
INSERT INTO ConfigSettings (Name, Description, Visible, AllowMultiple, DataType, Parent, Label, OrderNumber) SELECT 'tempPath', 'Path for temporary files', 1, 0, 'text', ID, 'Temp File Path', 16 FROM ConfigSettings WHERE Name="paths";
